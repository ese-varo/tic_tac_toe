...
def continue_playing(board, player1, player2, trackers)
  if trackers[:winner]
    puts "Congratulations #{winner(player1, player2, trackers)}!! You are the winner!!"
  else
    puts "Draw!!"
  end
  play_again(board, player1, player2, trackers)
end

def winner(player1, player2, trackers)
  player1.turn? ? trackers[:last_winner] = player2.name : trackers[:last_winner] = player1.name
end

def play_again(board, player1, player2, trackers)
